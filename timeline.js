...
function getItemAtPoint(x, y) {
  for (const rect of itemRects) {
    if (x >= rect.barX && x <= rect.barX + rect.barW && y >= rect.barY && y <= rect.barY + rect.barH) {
      return items.find(it => it.id === rect.id) || null;
    }
  }
  return null;
}
...
async function getWikipediaUrl(wdId, preferLang) {
  const cacheKey = `${wdId}:${preferLang || 'en'}`;
  if (wikipediaUrlCache.has(cacheKey)) return wikipediaUrlCache.get(cacheKey);
  ...
  try {
    ...
    if (title) {
      ...
      const wpUrl = `https://${wpLang}.wikipedia.org/wiki/${encodeURIComponent(title)}`;
      wikipediaUrlCache.set(cacheKey, wpUrl);
      return wpUrl;
    }
  } catch {}
  wikipediaUrlCache.set(cacheKey, '');
  return '';
}

async function openWikipedia(wdId, preferLang) {
  const wpUrl = await getWikipediaUrl(wdId, preferLang);
  if (wpUrl) {
...
canvas.addEventListener('touchmove', (e) => {
  ...
  if (e.touches.length === 1 && isDragging) {
    ...
    if (lastTouchDist > 0) {
      ...
      const nextView = clampView(yc + (viewStart - yc) * scale, yc - (yc - viewEnd) * scale);
      viewStart = nextView.start;
      viewEnd = nextView.end;
    }
    lastTouchDist = dist;
...
canvas.addEventListener('touchend', (e) => {
  ...
  const wasTap = touch && Math.abs(touch.clientX - touchStartX) <= 8 && Math.abs(touch.clientY - touchStartY) <= 8;
  isDragging = false;
  lastTouchDist = 0;
  if (!wasTap) return;
...
function renderTooltip(found, wpUrl = null) {
  if (isDragging) { tooltip.style.display = 'none'; return; }
  if (!found) { hideTooltip(); return; }
  if (wpUrl === null) wpUrl = wikipediaUrlCache.get(`${found.wdId}:${found.wpLang || 'en'}`);
  ...
  if (wpUrl) {
    ...
    link.textContent = lang === 'ru' ? 'Открыть Wikipedia' : 'Open Wikipedia';
    tooltip.appendChild(link);
  }
  tooltip.style.display = 'block';

  let tx = activeTooltipX + 16;
  let ty = activeTooltipY - 10;
  if (tx + tooltip.offsetWidth > W - 10) tx = activeTooltipX - tooltip.offsetWidth - 10;
  if (ty + tooltip.offsetHeight > H - 10) ty = activeTooltipY - tooltip.offsetHeight - 10;
  tx = Math.max(8, Math.min(tx, W - tooltip.offsetWidth - 8));
  ty = Math.max(8, Math.min(ty, H - tooltip.offsetHeight - 8));
  tooltip.style.left = tx + 'px';
  tooltip.style.top = ty + 'px';
}

function updateTooltip() {
  if (!activeTooltipItemId) {
    tooltip.style.display = 'none';
    return;
  }
